...
public class DataInitializer implements CommandLineRunner {
        ...
        @Autowired
        public void setSelf(@Lazy DataInitializer self) {
                this.self = self;
        }
        ...
        public void run(String... args) {
                ...
                if (hotelRepository.count() > 0) {
                        ...
                        self.createHotelOwnersForExistingHotels();
                        return;
                }
        ...
        private List<Amenity> createAmenities() {
                ...
                List<Amenity> amenities = Arrays.asList(Amenity.builder().name("Free WiFi").icon("wifi").build(),
        ...
        private void createHotels(List<User> hotelOwners, List<Amenity> allAmenities) {
                ...
                // 1. Luxury Hotel
                Set<Amenity> luxuryAmenities = new HashSet<>(Arrays.asList(allAmenities.get(0), // WiFi
                ...
                Hotel luxuryHotel = Hotel.builder().name("Grand Luxury Hotel").description(
                                "Experience the epitome of luxury in our 5-star hotel located in the heart of the city. Featuring elegant rooms, world-class dining, and exceptional service.")
                                .address("123 Luxury Avenue").city("Dubai").country("UAE").postalCode("10001")
                                .latitude(new BigDecimal("40.7128")).longitude(new BigDecimal("-74.0060"))
                                .starRating((short) 5).owner(hotelOwners.get(0)) // Assign
                                                                                 // luxury
                                                                                 // owner
                                .amenities(luxuryAmenities).build();
                ...
                // 2. Beach Resort
                Set<Amenity> beachAmenities = new HashSet<>(Arrays.asList(allAmenities.get(0), // WiFi
                                allAmenities.get(1), // Pool
                                allAmenities.get(3), // Spa
                                allAmenities.get(4), // Restaurant
                                allAmenities.get(5), // Bar
                ...
                Hotel beachResort = Hotel.builder().name("Seaside Paradise Resort").description(
                                "Escape to our beachfront paradise with stunning ocean views, private beach access, and tropical-inspired accommodations. Perfect for a relaxing getaway.")
                                .address("456 Beachfront Drive").city("Miami").country("USA").postalCode("33139")
                                .latitude(new BigDecimal("25.7617")).longitude(new BigDecimal("-80.1918"))
                                .starRating((short) 4).owner(hotelOwners.get(1)) // Assign
                                                                                 // beach
                                                                                 // owner
                                .amenities(beachAmenities).build();
                ...
                // 3. Boutique Hotel
                Set<Amenity> boutiqueAmenities = new HashSet<>(Arrays.asList(allAmenities.get(0), // WiFi
                                allAmenities.get(4), // Restaurant
                                allAmenities.get(5), // Bar
                                allAmenities.get(11) // Concierge
                ));

                Hotel boutiqueHotel = Hotel.builder().name("Artisan Boutique Hotel").description(
                                "A charming boutique hotel offering a unique blend of historic architecture and modern design. Each room is individually decorated with local artwork.")
                                .address("789 Artistic Lane").city("Paris").country("France").postalCode("75001")
                                .latitude(new BigDecimal("48.8566")).longitude(new BigDecimal("2.3522"))
                                .starRating((short) 4).owner(hotelOwners.get(2)) // Assign
                                                                                 // boutique
                                                                                 // owner
                                .amenities(boutiqueAmenities).build();
                ...
                // 4. Business Hotel
                Set<Amenity> businessAmenities = new HashSet<>(Arrays.asList(allAmenities.get(0), // WiFi
                                allAmenities.get(2), // Fitness
                                allAmenities.get(4), // Restaurant
                ...
                Hotel businessHotel = Hotel.builder().name("Executive Business Hotel").description(
                                "Designed for the modern business traveler, our hotel offers spacious work areas, high-speed internet, and convenient meeting facilities.")
                                .address("101 Corporate Plaza").city("London").country("UK").postalCode("EC1A 1BB")
                                .latitude(new BigDecimal("51.5074")).longitude(new BigDecimal("-0.1278"))
                                .starRating((short) 4).owner(hotelOwners.get(3)) // Assign
                                                                                 // business
                                                                                 // owner
                                .amenities(businessAmenities).build();
                ...
                // 5. Family Resort
                Set<Amenity> familyAmenities = new HashSet<>(Arrays.asList(allAmenities.get(0), // WiFi
                                allAmenities.get(1), // Pool
                                allAmenities.get(4), // Restaurant
                                allAmenities.get(7), // Parking
                ...
                Hotel familyResort = Hotel.builder().name("Family Fun Resort").description(
                                "The perfect destination for family vacations with activities for all ages, spacious family rooms, and a dedicated kids' club.")
                                .address("202 Family Circle").city("Orlando").country("USA").postalCode("32801")
                                .latitude(new BigDecimal("28.5383")).longitude(new BigDecimal("-81.3792"))
                                .starRating((short) 3).owner(hotelOwners.get(4)) // Assign
                                                                                 // family
                                                                                 // owner
                                .amenities(familyAmenities).build();
        ...
        public void createHotelOwnersForExistingHotels() {
                ...
                Optional<Hotel> luxuryHotelOpt = hotelRepository.findAll().stream().filter(
                                h -> h.getName().contains("Grand Luxury Hotel") || h.getName().contains("Luxury"))
                                .findFirst();
                ...
                Optional<Hotel> boutiqueHotelOpt = hotelRepository.findAll().stream().filter(
                                h -> h.getName().contains("Artisan Boutique") || h.getName().contains("Boutique"))
                                .findFirst();
                ...
                Optional<Hotel> businessHotelOpt = hotelRepository.findAll().stream().filter(
                                h -> h.getName().contains("Executive Business") || h.getName().contains("Business"))
                                .findFirst();
        ...
        private void createRooms(Hotel hotel) {
                ...
                Room standardRoom = Room.builder().hotel(hotel).name("Standard Room")
                                .description("Comfortable room with all the essential amenities for a pleasant stay.")
                                .capacity(2).pricePerNight(new BigDecimal("100.00")).roomCount(10) // Added
                                                                                                   // roomCount
                                                                                                   // field
                                .build();
                ...
                // Create deluxe room
                Room deluxeRoom = Room.builder().hotel(hotel).name("Deluxe Room")
                                .description("Spacious room with premium amenities and a beautiful view.").capacity(2)
                                .pricePerNight(new BigDecimal("150.00")).roomCount(8) // Added
                                                                                      // roomCount
                                                                                      // field
                                .build();
                ...
                // Create suite
                Room suite = Room.builder().hotel(hotel).name("Executive Suite")
                                .description("Luxurious suite with separate living area and exclusive amenities.")
                                .capacity(3).pricePerNight(new BigDecimal("250.00")).roomCount(5) // Added
                                                                                                  // roomCount
                                                                                                  // field
                                .build();
                ...
                // Create family room
                Room familyRoom = Room.builder().hotel(hotel).name("Family Room").description(
                                "Spacious room designed for families, with multiple beds and child-friendly features.")
                                .capacity(4).pricePerNight(new BigDecimal("200.00")).roomCount(6) // Added
                                                                                                  // roomCount
                                                                                                  // field
                                .build();
        ...
        private void createHotelImages(Hotel hotel, String type) {
                ...
                Image primaryImage = Image.builder().entityType("HOTEL").entityId(hotel.getId())
                                .url("/images/hotel-" + type + "-1.jpg").isPrimary(true).build();

                imageRepository.save(primaryImage);

                // Create additional images
                for (int i = 2; i <= 3; i++) {
                        Image image = Image.builder().entityType("HOTEL").entityId(hotel.getId())
                                        .url("/images/hotel-" + type + "-" + i + ".jpg").isPrimary(false).build();
